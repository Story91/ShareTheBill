...
export class BillNotifications {
  ...
  static async notifyPaymentReceived(
    ...
    for (const participant of bill.participants) {
      if (participant.fid !== paidParticipant.fid && participant.fid !== bill.creatorFid) {
        notifications.push(
          sendFrameNotification({
            fid: participant.fid,
            title: "Someone Paid! ✅",
  ...
  static async notifyBillCompleted(bill: Bill): Promise<void> {
    ...
    const allParticipants = [bill.creatorFid, ...bill.participants.map(p => p.fid)];
      ...
      if (daysDiff <= 1) {
        ...
        for (const participant of pendingParticipants) {
          ...
          notifications.push(
            sendFrameNotification({
              fid: participant.fid,
              title: daysDiff < 0 ? "Payment Overdue! ⚠️" : "Payment Due Tomorrow! ⏰",
              body: message
            }).then(() => {})
          );
        }
